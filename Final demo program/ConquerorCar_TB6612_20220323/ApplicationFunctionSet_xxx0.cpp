...
// DeviceDriverSet_RBGLED AppRBG_LED; // REMOVED - FastLED stripped
...
/*
  AutoLevel mode:
  Uses accelerometer pitch to keep the Y servo level.
  Runs at ~50Hz, non-blocking.
*/
extern MPU6050 accelgyro; // defined in MPU6050_getdata.cpp
...
void ApplicationFunctionSet::ApplicationFunctionSet_AutoLevel(void)
{
  ...
  // Read raw accelerometer for pitch
  ...
  if (fabs(smoothed_Pitch) > DEADBAND)
  {
    ...
    // Hysteresis: only move if change >= 2 degrees
...
void ApplicationFunctionSet::ApplicationFunctionSet_IRrecv(void)
{
  ...
  static int ir_z_angle = 90; // shared Z servo angle for IR control
  ...
  if (true == IRrecv_en)
  {
    ...
    case /* constant-expression */ 6:
      /* code */ // Z servo snap to 50
      ir_z_angle = 50;
      AppServo.DeviceDriverSet_Servo_controls(1, ir_z_angle);
      break;
    case /* constant-expression */ 7:
      /* code */ // Z servo increment toward 50 - continuous
      ir_z_angle -= 5;
      if (ir_z_angle < 50) ir_z_angle = 50;
      AppServo.DeviceDriverSet_Servo_controls(1, ir_z_angle);
      break;
    case /* constant-expression */ 8:
      /* code */ // Y servo snap to 180
      AppServo.DeviceDriverSet_Servo_controls(2, 180);
      break;
    case /* constant-expression */ 9:
      /* code */ // Z servo snap to 140
      ir_z_angle = 140;
      AppServo.DeviceDriverSet_Servo_controls(1, ir_z_angle);
      break;
    case /* constant-expression */ 10:
      /* code */ // Z servo increment toward 140 - continuous
      ir_z_angle += 10;
      if (ir_z_angle > 140) ir_z_angle = 140;
      AppServo.DeviceDriverSet_Servo_controls(1, ir_z_angle);
      break;
    case /* constant-expression */ 11:
      /* code */ // Y servo snap to 118
      AppServo.DeviceDriverSet_Servo_controls(2, 118);
      break;
    ...
    /*方向控制部分实现时长约束控制*/
    // Buttons 1-4 (drive) and 7,10 (servo continuous) use timeout
    if (IRrecv_button < 5 || IRrecv_button == 7 || IRrecv_button == 10)
    {
      if (IRrecv_button < 5)
