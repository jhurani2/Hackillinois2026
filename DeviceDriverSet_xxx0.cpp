...
void DeviceDriverSet_Servo::DeviceDriverSet_Servo_Init(unsigned int Position_angle) {
  Position_angle = constrain(Position_angle, 0, 180);

  servo_z.attach(PIN_Servo_z);
  servo_y.attach(PIN_Servo_y);

  servo_z.write(Position_angle);
  servo_y.write(Position_angle);
}

void DeviceDriverSet_Servo::DeviceDriverSet_Servo_controls(uint8_t ServoID,
                                                           unsigned int Position_angle) {
  Position_angle = constrain(Position_angle, 0, 180);

  if (ServoID == 1 || ServoID == 3) {  // Z servo
    servo_z.write(Position_angle);
  }

  if (ServoID == 2 || ServoID == 3) {  // Y servo
    servo_y.write(Position_angle);
  }
}
